...
fun List<MethodNode>.named(name: String) = find { it.name == name }
    ?: error("Could not find a method with name $name")

fun List<MethodNode>.search(name: String, desc: String) = find { it.name == name && it.desc == desc }
    ?: error("Could not find a method with name $name and desc $desc")

fun List<MethodNode>.search(name: String, returnType: String, vararg args: String) =
    find { it.name == name && it.desc == "(${args.joinToString("")})$returnType" }
        ?: error("Could not find a method with name $name, returnType $returnType and args $args")

fun List<FieldNode>.named(name: String) = find { it.name == name }
    ?: error("Could not find a field with name $name")

fun List<FieldNode>.search(name: String, type: String) = find { it.name == name && it.desc == type }
    ?: error("Could not find a field with name $name and desc $type")
...
fun ByteArray.dump(file: String) = FileOutputStream(file).use { it.write(this) }
