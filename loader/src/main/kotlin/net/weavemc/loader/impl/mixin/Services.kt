...
import net.weavemc.loader.impl.util.javaVersion
...
internal class SandboxedMixinService : IMixinService {
    ...
    override fun offer(internal: IMixinInternal) {
        if (internal is IMixinTransformerFactory) {
            if (sandboxLoader == null) {
                logger.warn("SandboxedMixinService was loaded within illegal parent loader $sandboxLoader")
            }
    ...
    override fun getMaxCompatibilityLevel() = runCatching {
        MixinEnvironment.CompatibilityLevel.valueOf("JAVA_${javaVersion}")
    }.getOrNull()
...
internal class WeaveLoggerAdapter(name: String) : LoggerAdapterAbstract(name) {
    private val logger by klog(name)
    private val stdout = WeaveLogAppender.WrappingStream(System.out)

    override fun getType() = "Default Console Logger"
    override fun catching(level: Level, t: Throwable) = logger.warn("Catching {}: {}", t.javaClass.name, t.message, t)
    override fun log(level: Level, message: String, vararg params: Any) {
        ...
        if (formatted.hasThrowable()) formatted.throwable.printStackTrace(stdout)
    }
